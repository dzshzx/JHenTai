...
class FirstOpenHandler implements UpdateHandler {
  ...
  Future<void> onReady() async {
    ...
    localConfigService.write(
      configKey: ConfigEnum.firstOpenInited,
      value: 'true',
    );
  }
}
...
class StyleSettingMigrateHandler implements UpdateHandler {
  ...
  Future<void> onInit() async {
    ...
    Map<String, dynamic>? styleSettingMap = storageService
        .read<Map<String, dynamic>>(ConfigEnum.styleSetting.key);

    if (styleSettingMap?['locale'] != null) {
      preferenceSetting.saveLanguage(
        localeCode2Locale(styleSettingMap!['locale']),
      );
    }
    ...
    if (styleSettingMap?['showR18GImageDirectly'] != null) {
      preferenceSetting.saveShowR18GImageDirectly(
        styleSettingMap!['showR18GImageDirectly'],
      );
    }
    ...
    if (await downloadDir.exists()) {
      downloadDir.list().listen(
        (entity) async {
          ...
          File oldArchiveMetadataFile = File(
            join(entity.path, '.archive.metadata'),
          );
...
class UpdateLocalGalleryPathHandler implements UpdateHandler {
  ...
  Future<void> onInit() async {
    ...
    downloadSetting.removeExtraGalleryScanPath(
      downloadSetting.defaultDownloadPath,
    );
  }
...
class MigrateSearchConfigHandler implements UpdateHandler {
  ...
  Future<void> onReady() async {
    ...
    Map<String, dynamic>? map =
        ...
        storageService.read(
          '${ConfigEnum.searchConfig.key}: SearchPageMobileV2Logic',
        );
    if (map != null) {
...
class MigrateCookieHandler implements UpdateHandler {
  ...
  Future<void> onInit() async {
    ...
    Map<String, Map<String, dynamic>> cookieStrs = json
        .decode(str)
    ...
    for (int i = 0; i < pageCount; i++) {
      try {
        ...
        for (GalleryHistoryData history in oldHistories) {
          Future future = isolateService
              .jsonDecodeAsync(history.jsonBody)
              .then((map) {
                return Gallery.fromJson(map);
              })
              .then((gallery) {
                return isolateService.jsonEncodeAsync(
                  gallery2GalleryHistoryModel(gallery),
                );
              })
              .then((galleryString) {
...
class MigrateStorageConfigHandler implements UpdateHandler {
  ...
  Future<void> onInit() async {
    ...
    Map? favoriteSettingMap = storageService.read(
      ConfigEnum.favoriteSetting.key,
    );
    ...
    Map? advancedSettingMap = storageService.read(
      ConfigEnum.advancedSetting.key,
    );
    ...
    Map? downloadSettingMap = storageService.read(
      ConfigEnum.downloadSetting.key,
    );
    ...
    Map? performanceSettingMap = storageService.read(
      ConfigEnum.performanceSetting.key,
    );
    ...
    Map? preferenceSettingMap = storageService.read(
      ConfigEnum.preferenceSetting.key,
    );
    ...
    Map? securitySettingMap = storageService.read(
      ConfigEnum.securitySetting.key,
    );
    ...
    double? leftColumnWidthRatio = storageService.read(
      ConfigEnum.leftColumnWidthRatio.key,
    );
    ...
    if (dismissVersion != null) {
      futures.add(
        localConfigService.write(
          ...
          value: dismissVersion,
        ),
      );
    }

    int? downloadPageBodyType = storageService.read(
      ConfigEnum.downloadPageBodyType.key,
    );
  ...
  Future<void> onReady() async {
    ...
    if (keys != null) {
      ...
      if (gid2ReadIndexMap.isNotEmpty) {
        List<LocalConfigCompanion> localConfigs = gid2ReadIndexMap.entries.map((
          entry,
        ) {
