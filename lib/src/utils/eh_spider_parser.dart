...
class EHSpiderParser {
    ...
    if (success) {
      map['ipbMemberId'] = int.parse(
        RegExp(r'ipb_member_id=(\d+);')
            .firstMatch(
              cookieHeaders.firstWhere(
                (header) => header.contains('ipb_member_id'),
              ),
            )!
            .group(1)!,
      );
      map['ipbPassHash'] = RegExp(r'ipb_pass_hash=(\w+);')
          .firstMatch(
            cookieHeaders.firstWhere(
              (header) => header.contains('ipb_pass_hash'),
            ),
          )!
          .group(1)!;
    } else {
  ...
  static GalleryPageInfo _minimalGalleryPageDocument2GalleryPageInfo(
    ...
    List<Element> galleryListElements = document.querySelectorAll(
      '.itg.gltm > tbody > tr',
    );
  ...
  static int? _ranklistPageDocument2PrevPageIndex(Document document) {
    ...
    return int.tryParse(
      RegExp(
            r'p(age)?=(\d+)',
          ).firstMatch(a.attributes['href'] ?? '')?.group(2) ??
          '0',
    );
  }
  ...
  detailPage2GalleryAndDetailAndApikey(Headers headers, dynamic data) {
    ...
    int pageCount = int.parse(
      (document
                  .querySelector(
                    '#gdd > table > tbody > tr:nth-child(5) > .gdt2',
                  )
                  ?.text ??
              '')
          .split(' ')[0],
    ...
    LinkedHashMap<String, List<GalleryTag>> tags = _detailPageDocument2Tags(
      document,
    );
    String language =
        document
    ...
    bool isExpunged =
        (document
                    .querySelector(
                      '#gdd > table > tbody > tr:nth-child(2) > .gdt2',
                    )
                    ?.text ??
                '')
            .contains('Expunged');
    ...
    GalleryDetail galleryDetail = GalleryDetail(
      ...
      ratingCount: int.parse(
        document.querySelector('#rating_count')?.text ?? '0',
      ),
      size:
          document
              .querySelector('#gdd > table > tbody')
  ...
  static LinkedHashMap<String, List<GalleryTag>> _detailPageDocument2Tags(
    ...
    List<Element> trs = document
        .querySelectorAll('#taglist > table > tbody > tr')
        .toList();
    for (Element tr in trs) {
      List<Element> tagDivs = tr
          .querySelectorAll('td:nth-child(1) > div')
          .toList();
  ...
  static Map<String, List> favoritePage2FavoriteTagsAndCounts(
    ...
    for (Element div in divs) {
      ...
      int favoriteCount = int.parse(
        div.querySelector('div:first-child')?.text ?? '0',
      );
  ...
  static GalleryImage imagePage2GalleryImage(Headers headers, dynamic data) {
    ...
    String imageHash = RegExp(
      r'f_shash=(\w+)',
    ).firstMatch(hashElement.attributes['href']!)!.group(1)!;
    ...
    double? originalImgWidth = double.tryParse(
      originalImgWidthAndHeight?.group(1) ?? '',
    );
    double? originalImgHeight = double.tryParse(
      originalImgWidthAndHeight?.group(2) ?? '',
    );

    /// return nl('WZG-474997')
    Element reloadKeyElement = document.querySelector('#loadfail')!;
    String reloadKey = RegExp(
      r"return nl\('(.*)'\)",
  ...
  static List<GalleryTorrent> torrentPage2GalleryTorrent(
    ...
    List<Element> torrentForms = document.querySelectorAll(
      '#torrentinfo > div > form',
    );
    ...
    torrentForms.removeWhere(
      (form) =>
          form.querySelector(
            'div > table > tbody > tr:nth-child(4) > td > a',
          ) ==
          null,
    );
  ...
  myTagsPage2TagSetNamesAndTagSetsAndApikey(Headers headers, dynamic data) {
    ...
    List<Element> options = document.querySelectorAll(
      '#tagset_outer > div > select > option',
    );
    ...
    List<WatchedTag>
    tags = tagDivs.where((element) => element.id != 'usertag_0').map((div) {
      String pair =
          div
      ...
      return WatchedTag(
        ...
        backgroundColor: aRGBString2Color(
          div.querySelector('div:nth-child(9) > input')?.attributes['value'],
        ),
        weight: int.parse(
          div.querySelector('div:nth-child(11) > input')!.attributes['value']!,
        ),
      );
    }).toList();
  ...
  static GalleryStats statPage2GalleryStats(Headers headers, dynamic data) {
    ...
    Element? rankScoreTbody = document.querySelector(
      '.stuffbox > table > tbody',
    );
  ...
  static GalleryArchive archivePage2Archive(Headers headers, dynamic data) {
    ...
    return GalleryArchive(
      ...
      originalSize: document
          .querySelector('#db > div > div > p > strong')!
          .text,
  ...
  static Map<String, String> exchangePage2Assets(
    ...
    String? creditDesc = document
        .querySelector('#buyform')
        ?.parent
        ?.nextElementSibling
        ?.text;
    String? gpCreditDesc = document
        .querySelector('#sellform')
        ?.parent
        ?.nextElementSibling
        ?.text;

    String? credit = RegExp(
      r'([\d,k ]+)Credits',
    ).firstMatch(creditDesc ?? '')?.group(1);
    String? gp = RegExp(
      r'([\d,k ]+)GP',
    ).firstMatch(gpCreditDesc ?? '')?.group(1);
  ...
  static Gallery _parseExtendedGallery(Element tr) {
    LinkedHashMap<String, List<GalleryTag>> tags = _parseExtendedGalleryTags(
      tr,
    );
  ...
  static Gallery _parseThumbnailGallery(Element div) {
    ...
    Gallery gallery = Gallery(
      galleryUrl: GalleryUrl.parse(
        div.querySelector('a')?.attributes['href'] ?? '',
      ),
      ...
      favoriteTagName: div
          .querySelector('.gl5t > div > [id][style]')
          ?.attributes['title'],
      tags: LinkedHashMap(),
  ...
  static String _parsePostedLocalTime(Element element) {
    ...
    String postedTimeString =
        RegExp(
          r'Posted on (.+, .+)( by:)?',
        ).firstMatch(postedTimeDesc)?.group(1) ??
        '';
  ...
  static List<GalleryThumbnail> _parseGalleryDetailsForNewThumbnails(
    ...
    return thumbNailElements.map((element) {
      ...
      if (thumbUrl.startsWith('/')) {
        GalleryImagePageUrl galleryImagePageUrl = GalleryImagePageUrl.parse(
          href,
        );
        thumbUrl =
