...
- If a Flutter build hits a network timeout while running native asset hooks, first retry with `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and lowercase variants set to `http://127.0.0.1:7897`. Stop stale Gradle daemons before retrying so hook subprocesses inherit the proxy environment. Do not change hook configuration or bypass the default native asset path unless the proxy is unavailable and the default path still fails.
